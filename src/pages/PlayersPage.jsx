...
const PlayersPage = () => {
        ...
        const fetchAllData = async () => {
            try {
                const [playersRes, teamsRes] = await Promise.all([
                    axios.get(`http://localhost:30002/back2/api/players`),
                    axios.get(`http://localhost:30002/back2/api/teams`)
                ]);
    ...
    const handleSavePlayer = async () => {
        ...
        try {
            if (isEditing) {
                await axios.put(`http://localhost:30002/back2/api/players/${currentPlayer.id}`, playerToSave);
                setSuccessMessage("Player updated successfully!");
            } else {
                await axios.post(`http://localhost:30002/back2/api/players`, playerToSave);
                setSuccessMessage("Player added successfully!");
            }
            const response = await axios.get(`http://localhost:30002/back2/api/players`);
    ...
    const handleDeletePlayer = async (id) => {
        try {
            await axios.delete(`http://localhost:30002/back2/api/players/${id}`);
